...
public class ScriptResourceUtil {
    ...
    public static BufferedImage grabFrame(ResourceLocation id, int frameNumber) throws Exception {
        InputStream inputStream = readStream(id);
        ...
        try {
            ...
            for (int i = 0; i < frameNumber && frame != null; i++) {
    ...
    public static List<BufferedImage> grabFrames(ResourceLocation id) throws Exception {
        InputStream inputStream = readStream(id);
        FFmpegFrameGrabber frameGrabber = new FFmpegFrameGrabber(inputStream);
        try {
            frameGrabber.start();
            ...
            for (Frame f : frames) {
                bufferedImages.add(converter.convert(f));
            }
