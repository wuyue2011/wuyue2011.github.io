function fetchIpLocation(callback) {
    ...
    const key = 'WNXBZ-LH6W7-ECHXK-PGNHX-3GWOK-HWB2N'; // 请修改为你申请到的key
    const url = `https://apis.map.qq.com/ws/location/v1/ip?key=${key}&output=jsonp&callback=callbackFunction`;
...
function showWelcome() {
    if (!ipLocation) {
        fetchIpLocation(showWelcome);
        return;
    }
    let dist = getDistance(118.16799114155504, 25.059852248706417, ipLocation.result.location.lng, ipLocation.result.location.lat); //这里记得换成自己的经纬度
...
// window.onload = fetchIpLocation(showWelcome);
