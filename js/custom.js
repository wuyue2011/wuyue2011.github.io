...
// 进行 fetch 请求
fetch('https://api.76.al/api/ip/query?key=CF40OWzXZN5XZZaaOCzQSHKvXw') //申请key:https://api.76.al
.then(response => {
if (!response.ok) {
...
return;
 ...
 let dist = getDistance(118.1817065, 25.0582843, ipLocation.data.lng, ipLocation.data.lat); // 修改自己的经度（121.413921）纬度（31.089290）
 ...
 if (welcomeInfoElement) {
   ...
   document.addEventListener('DOMContentLoaded', function () {
    ...
    async function fetchHistoryData() {
        ...
        const historyDataUrl = `https://api.76.al/api/history/query?key=CF40OWzXZN5XZZaaOCzQSHKvXw`; //请到:https://api.76.al申请
